...
def checkNewStuff(day):
  ...
  newMPs = [x for x in MoviePeople.objects.filter(date_info=day) if not (Release.objects.filter(movie=x.movie, country='US')[0].date < date)]
  newTrailers_movies = [(x.movie, x.url) for x in Trailer.objects.filter(date_info=day) if not (Release.objects.filter(movie=x.movie, country='US')[0].date < date)]
  newTrailers_MPs = [(MoviePeople.objects.filter(movie=x[0]), x[0], x[1]) for x in newTrailers_movies]
  newReleases_movies = [(x.movie, date + datetime.timedelta(7)) for x in Release.objects.filter(date=date + datetime.timedelta(7), country='US') if not (Release.objects.filter(movie=x.movie, country='US')[0].date < date)]
